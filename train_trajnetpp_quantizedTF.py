...
def main():
    ...
    ############################################################################################################
            ...
            loss.backward()
            optim.step()
            print("epoch %03i/%03i  frame %04i / %04i loss: %7.4f" % (epoch, args.max_epoch, id_b, len(train_dataset), loss.item()))
    ...
    ## TrajNet++ Edit ################################################################################################
            ...
            for _, (filename, scene_id, paths) in enumerate(val_dataset):
                ## make new scene
                pos_scene = trajnetplusplustools.Reader.paths_to_xy(paths)[:, 0]  # primary ped
                vel_scene = np.zeros_like(pos_scene)
                vel_scene[1:] = pos_scene[1:] - pos_scene[:-1]
    ...
    ############################################################################################################
                ...
                epoch, args.max_epoch, step, len(val_dataset), loss.item()))
                val_loss+=loss.item()
                step+=1
    ## TrajNet++ Edit ################################################################################################
                batch = {'src': [], 'trg': []}
